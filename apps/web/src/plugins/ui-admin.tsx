...
export const uiAdminPlugin: Plugin = {
    name: "ui-admin",
    description: "简易后台管理(用户 / 群组 / 系统)",
    inject: ["ui", "auth", "rpc"],
    async apply(ctx) {
        ...
        const useIsAdmin = () => {
            ...
            useEffect(() => {
                check();
                const dispose = rpc.onStatus((status) => {
                    if (status === "open") check();
                });
                return () => {
                    void dispose();
                };
        ...
        const AdminPage = () => {
            const isAdmin = useIsAdmin();
            const [tab, setTab] = useState<"users" | "groups" | "system">(
                "users",
            );
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                {groups.map((group) => (
                                    <div
                                        ...
                                        className="flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-accent/60"
                                    >
                                        <span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-sky-400 text-white">
        ...
        const unregisterNav = ui.register("nav", AdminNav);
