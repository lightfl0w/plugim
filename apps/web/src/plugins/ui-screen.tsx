...
export const uiScreenSetup = async (ctx: Context) => {
    ...
    (window as unknown as { __screenLog: string[] }).__screenLog = screenLog;
    ...
    const applyShareEncodings = () => {
        for (const sender of pc?.getSenders() ?? []) {
            ...
            void sender
                ...
                .then(() =>
                    trace(
                        `encodings video maxBitrate=${encoding.maxBitrate ?? "-"}`,
                    ),
                )
                .catch(() => undefined);
        }
    ...
    const beginShare = async () => {
        ...
        try {
            ...
            videoTrack.onended = () => void hangup();
            for (const transceiver of pc.getTransceivers()) {
                ...
                try {
                    transceiver.setCodecPreferences([
                        ...h264,
                        ...caps.codecs.filter((codec) => !isH264(codec)),
                    ]);
